...
#ifndef _UTP_SBE_MDFULLREFRESH_CXX_H_
#define _UTP_SBE_MDFULLREFRESH_CXX_H_
...
public:
    ...
    MDFullRefresh(char *buffer, const std::uint64_t bufferLength) :
        MDFullRefresh(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDFullRefresh(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDFullRefresh(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MDFullRefresh &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MDFullRefresh &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MDFullRefresh skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    static SBE_CONSTEXPR std::int64_t lastMsgSeqNumProcessedNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t lastMsgSeqNumProcessedMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t lastMsgSeqNumProcessedMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }
    ...
    SBE_NODISCARD std::int64_t lastMsgSeqNumProcessed() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    MDFullRefresh &lastMsgSeqNumProcessed(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    MDFullRefresh &securityID(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    static SBE_CONSTEXPR std::int64_t rptSeqNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t rptSeqMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t rptSeqMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }
    ...
    SBE_NODISCARD std::int64_t rptSeq() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    MDFullRefresh &rptSeq(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    MDFullRefresh &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    MDFullRefresh &mDEntryOriginator(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    MDFullRefresh &putMDEntryOriginator(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 28, src, sizeof(char) * 16);
        return *this;
    }
    ...
    #if __cplusplus >= 201703L
    MDFullRefresh &putMDEntryOriginator(const std::string_view str)
    {
    ...
    #else
    MDFullRefresh &putMDEntryOriginator(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    SBE_NODISCARD std::uint8_t marketDepth() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    MDFullRefresh &marketDepth(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    {
        std::int8_t val;
        std::memcpy(&val, m_buffer + m_offset + 45, sizeof(std::int8_t));
        return (val);
    }
    ...
    MDFullRefresh &securityType(const MarketDataType::Value value) SBE_NOEXCEPT
    {
        std::int8_t val = (value);
        std::memcpy(m_buffer + m_offset + 45, &val, sizeof(std::int8_t));
        return *this;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MDFullRefresh &_writer)
{
    MDFullRefresh writer(
        _writer.m_buffer,
        _writer.m_offset,
