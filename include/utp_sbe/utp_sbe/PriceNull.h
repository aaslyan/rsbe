...
#ifndef _UTP_SBE_PRICENULL_CXX_H_
#define _UTP_SBE_PRICENULL_CXX_H_
...
public:
    ...
    PriceNull(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        PriceNull(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    PriceNull(
        char *buffer,
        const std::uint64_t bufferLength) :
        PriceNull(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    PriceNull &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, PriceNull &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
