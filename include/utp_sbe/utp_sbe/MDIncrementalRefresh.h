...
#ifndef _UTP_SBE_MDINCREMENTALREFRESH_CXX_H_
#define _UTP_SBE_MDINCREMENTALREFRESH_CXX_H_
...
public:
    ...
    MDIncrementalRefresh(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefresh(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefresh(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefresh(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MDIncrementalRefresh &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MDIncrementalRefresh &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MDIncrementalRefresh &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    MDIncrementalRefresh &sbeRewind()
    {
    ...
    {
        MDIncrementalRefresh skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    MDIncrementalRefresh &securityID(const std::int32_t value) SBE_NOEXCEPT
    {
    ...
    MDIncrementalRefresh &rptSeq(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    {
        std::uint64_t val;
        std::memcpy(&val, m_buffer + m_offset + 12, sizeof(std::uint64_t));
        return SBE_LITTLE_ENDIAN_ENCODE_64(val);
    }

    MDIncrementalRefresh &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 12, &val, sizeof(std::uint64_t));
        return *this;
    }
    ...
    MDIncrementalRefresh &mDEntryOriginator(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    MDIncrementalRefresh &putMDEntryOriginator(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 20, src, sizeof(char) * 16);
        return *this;
    }
    ...
    #if __cplusplus >= 201703L
    MDIncrementalRefresh &putMDEntryOriginator(const std::string_view str)
    {
    ...
    #else
    MDIncrementalRefresh &putMDEntryOriginator(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #pragma GCC diagnostic pop
        ...
        SBE_NODISCARD std::int8_t mDUpdateActionRaw() const SBE_NOEXCEPT
        {
            std::int8_t val;
        ...
        {
            char val;
            std::memcpy(&val, m_buffer + m_offset + 1, sizeof(char));
            return (val);
        }
        ...
        {
            char val = (value);
            std::memcpy(m_buffer + m_offset + 1, &val, sizeof(char));
            return *this;
        }
...
public:
        ...
        {
            m_mDEntryPx.wrap(m_buffer, m_offset + 2, m_actingVersion, m_bufferLength);
            return m_mDEntryPx;
        }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MDIncrementalRefresh &_writer)
{
    MDIncrementalRefresh writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefresh", )";
