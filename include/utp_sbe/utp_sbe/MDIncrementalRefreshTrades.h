...
#ifndef _UTP_SBE_MDINCREMENTALREFRESHTRADES_CXX_H_
#define _UTP_SBE_MDINCREMENTALREFRESHTRADES_CXX_H_
...
class MDIncrementalRefreshTrades
{
private:
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(111);
    ...
    MDIncrementalRefreshTrades() = default;

    MDIncrementalRefreshTrades(
        char *buffer,
    ...
    MDIncrementalRefreshTrades(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefreshTrades(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefreshTrades(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefreshTrades(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MDIncrementalRefreshTrades &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MDIncrementalRefreshTrades &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MDIncrementalRefreshTrades &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    MDIncrementalRefreshTrades &sbeRewind()
    {
    ...
    {
        MDIncrementalRefreshTrades skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    MDIncrementalRefreshTrades &securityID(const std::int32_t value) SBE_NOEXCEPT
    {
...
private:
    MonthYearDay m_tradeDate;

public:
    SBE_NODISCARD MonthYearDay &tradeDate()
    {
        m_tradeDate.wrap(m_buffer, m_offset + 4, m_actingVersion, m_bufferLength);
        return m_tradeDate;
    }
    ...
    MDIncrementalRefreshTrades &mDEntryOriginator(const std::uint64_t index, const char value)
    {
        if (index >= 16)
        {
    ...
    MDIncrementalRefreshTrades &putMDEntryOriginator(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    MDIncrementalRefreshTrades &putMDEntryOriginator(const std::string_view str)
    {
    ...
    #else
    MDIncrementalRefreshTrades &putMDEntryOriginator(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 16)
        {
    ...
    #pragma GCC diagnostic pop
        ...
        {
            return static_cast<std::int8_t>(MDUpdateAction::Value::NEW);
        }
...
public:
        ...
        {
            m_settlDate.wrap(m_buffer, m_offset + 8, m_actingVersion, m_bufferLength);
            return m_settlDate;
        }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MDIncrementalRefreshTrades &_writer)
{
    MDIncrementalRefreshTrades writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefreshTrades", )";
